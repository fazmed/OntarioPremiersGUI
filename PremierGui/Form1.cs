...
namespace PremierGui
{
    ...
    public partial class Form1 : Form
    {
        ...
        public void InitializeGui()
        {
            ...
            foreach (Premier premier in Premier.GetPremiers())
            {
                premiers.Add(premier.GenerateKey(), premier);
            }
        }
        public Form1()
        {
        ...
        private void lstPremier_SelectedIndexChanged(object sender, EventArgs e)
        {
                ...
                string curItem = lstPremier.SelectedItem.ToString();
                picPremier.Image = Image.FromFile("bin/Debug/images/" + curItem + ".jpg");
            foreach (var p in premiers)
            {
                if (p.Key.Equals(curItem))
                {
                    LblName.Text = p.Value.Name;
                    LblBirth.Text = p.Value.Life;
                    PremierStartEnd.Text = p.Value.Start + " - " + p.Value.End;
                }
            }
